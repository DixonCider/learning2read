...
def get_df_total(path):
    raw_train = eval(path)('btrain')
    raw_test = eval(path)('btest')
    raw_implicit = eval(path)('implicit')
    raw_user = eval(path)('user')
    raw_book = eval(path)('book')
    df_total = learning2read.preprocessing.TotalDataFrame.run([raw_train, raw_test, raw_implicit])['output']
    return df_total

def preprocessingForVAE(df_total):
    """
    Input : df_Total
