...
int main(int argc, char** argv) {
  if (argc < 7) {
    std::cout << "./MF track2 usrb bokb bias defu defb\n";
    return 1;
  }
  bool track2 = std::string(argv[1]) == "2";
  double usrb = std::stod(std::string(argv[2]));
  double bokb = std::stod(std::string(argv[3]));
  double bias = std::stod(std::string(argv[4]));
  double defu = std::stod(std::string(argv[5]));
  double defb = std::stod(std::string(argv[6]));
  ...
  for (auto& i : test) {
    ...
    if (pred > 10) pred = 10;
    if (pred < 1) pred = 1;
    if (track2) std::cout << pred << '\n';
    else std::cout << (int)(pred + 0.6) << '\n';
  }
}
